...
@section('page-title','ویرایش کوپن: ' . $coupon->code)
@section('content')
<div class="max-w-xl">
...
<div class="bg-white rounded-2xl shadow p-6 space-y-4">
    <div>
        ...
        <input type="text" value="{{ $coupon->code }}" disabled dir="ltr"
            class="w-full border-2 rounded-xl px-4 py-3 bg-gray-50 font-mono font-black text-green-700">
    </div>
    ...
    <div class="grid grid-cols-2 gap-4">
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">حداقل خرید</label>
            <input type="number" name="min_order" value="{{ old('min_order', $coupon->min_order) }}" min="0"
                class="w-full border-2 rounded-xl px-4 py-3 focus:ring-2 focus:ring-green-500 focus:outline-none">
        </div>
        <div>
            ...
            <input type="number" name="max_uses" value="{{ old('max_uses', $coupon->max_uses) }}" min="1"
                class="w-full border-2 rounded-xl px-4 py-3 focus:ring-2 focus:ring-green-500 focus:outline-none" placeholder="بی‌نهایت">
        </div>
    </div>
    <div>
        ...
        <input type="datetime-local" name="expires_at"
            ...
            class="w-full border-2 rounded-xl px-4 py-3 focus:ring-2 focus:ring-green-500 focus:outline-none" dir="ltr">
    </div>
    <label class="flex items-center gap-3 cursor-pointer">
        <input type="checkbox" name="active" value="1" {{ old('active', $coupon->active) ? 'checked' : '' }} class="w-5 h-5 accent-green-600">
